...
//mean fishsel
//fishsel[0]=0.2376;
fishsel[0]=0;fishsel[1]=1;fishsel[2]=1;
...
Q_b[0]=1;Q_b[1]=0;Q_b[2]=0;//Q_b[3]=0.62;Q_b[4]=0.86;Q_b[5]=1;Q_b[6]=1;

//Skagerrak cod maturity (Knutsen 2011)
Q_a[0]=1;Q_a[1]=0;Q_a[2]=0;
...
NumericMatrix fst_submat699(nsims,9);
NumericMatrix sig_submat699(nsims,9);
NumericMatrix fst_submat660(nsims,9);
NumericMatrix sig_submat660(nsims,9);
...
NumericVector tallyhowmanyA(nSNP);
...
M2[0]=500;
M2[1]=500;M2[2]=500;//M2[3]=0.233;M2[4]=0.2;M2[5]=0.2;M2[6]=0.2;//natural mortality from ICES report
...
double sigma_ja2A=0;log(((1/pop1_prop)*pow(CVa,2))+1);
double sigma_ja2B=0;log(((1-(1/pop1_prop))*pow(CVa,2))+1);
double sigma_ja2=0;log(pow(CVa,2)+1);
double sigma_js2A=0;log(((1/pop1_prop)*pow(CVs,2))+1);
double sigma_js2B=0;log(((1-(1/pop1_prop))*pow(CVs,2))+1);
double sigma_js2=0;log(pow(CVs,2)+1);

double sigma_jr2=0;log(pow(CVr,2)+1);//may want to check this
double psiA;
double psiB;
double psi_star;
double rho=0.9;
...
NumericMatrix arichAmat_sub660;
NumericMatrix arichBmat_sub660;
NumericMatrix arichAmat_sub680;
NumericMatrix arichBmat_sub680;
NumericMatrix arichAmat_sub699;
NumericMatrix arichBmat_sub699;
...
mu=0.13*as<double>(rbeta(1,0.6,1));//here scale down by 1/100 so max is 0.01 OR not  for 2 alleles use .0003 for 20 use .13
...
N_til_init_a[0]=1;N_til_init_a[1]=0;N_til_init_a[2]=0;}

//N_til_init_a[i]=N_til_init_a[i-1]*exp(-M2[i-1]);}
//N_til_init_a[ages-1]=N_til_init_a[ages-2]*exp(-M2[ages-2])/(1-exp(-M2[ages-1]));

Rcout<<"N_til_init_a"<<std::endl;
Rf_PrintValue(wrap(N_til_init_a));

//establish the SBPR with 1 recruit and F40%
N_til_init_a40[0]=1;N_til_init_a40[1]=0;N_til_init_a40[2]=0;
//for (int i=1; i < (ages-1); i++){  //fill in N_til_init;
...
Rcout<<"N_til_init_a40"<<std::endl;
Rf_PrintValue(wrap(N_til_init_a40));
...
Weight_b[0]=20.75;Weight_b[1]=100.5;Weight_b[2]=100.5;
Weight_a[0]=2.75;Weight_a[1]=10.5508;Weight_a[2]=10.1016;//Weight_a[3]=1.6524;
...
R_l0A[0]=5460;//2.0*S_l0[0]/std::accumulate(tempvec1a.begin(), tempvec1a.end(), 0.0); //initial number recruits pop1  NOTE: if only one pop, all are in popA
R_l0B[0]=67000;//2.0*S_l0[1]/std::accumulate(tempvec1b.begin(), tempvec1b.end(), 0.0); //init recruits pop2
...
Rcout<<"N_inita1,a2"<<std::endl;
Rf_PrintValue(wrap(N_init_a1));
Rf_PrintValue(wrap(N_init_a2));
...
psiA=0;//as<double>(rnorm(1,0,sqrt(sigma_js2A)));
psiB=0;//as<double>(rnorm(1,0,sqrt(sigma_js2B)));
psi_star=0;//as<double>(rnorm(1,0,sqrt(sigma_js2)));
...
Rcout<<"spawnersA_rounded_star"<<std::endl;
Rf_PrintValue(wrap(spawnersA_rounded_star));
...
Rcout<<"tallyAB"<<std::endl;
Rf_PrintValue(wrap(tallyAB));
...
//for (int kk=0;kk<n_sampA;k++){
//if ((arichAmat((2*kk),i)!=arichAmat(((2*kk)+1),i))&((arichAmat((2*kk),i)==talmat(j,0))|(arichAmat(((2*kk)+1),i)==talmat(j,0)))){hetsA=hetsA+1;}
//}
//for (int kk=0;kk<n_sampB;k++){
//if ((arichBmat((2*kk),i)!=arichBmat(((2*kk)+1),i))&((arichBmat((2*kk),i)==talmat(j,0))|(arichBmat(((2*kk)+1),i)==talmat(j,0)))){hetsB=hetsB+1;}
//}

//talmat(j,3)=hetsA/n_sampA;
//talmat(j,4)=hetsB/n_sampB;
//}}   

for (int kk=0;kk<talmat.nrow();kk++){
talmat(kk,3)=2*talmat(kk,1)*(1-talmat(kk,1));
talmat(kk,4)=2*talmat(kk,2)*(1-talmat(kk,2));
}


talmat2=clone(talmat);
Rcout<<"talmat2"<<std::endl;
Rf_PrintValue(wrap(talmat2));

//talmat is the raw material to find Fst!!!
//GET FST this function finds Fst between 2 populations, multiple loci and multiple alleles.

Rcout<<"n_sampA,B"<<std::endl;
Rf_PrintValue(wrap(n_sampA));
Rf_PrintValue(wrap(n_sampB));


double n_bar=0.5*(n_sampA+n_sampB); //sample size - can change this.
double r=2;
Rcout<<"n_bar,r"<<std::endl;
Rf_PrintValue(wrap(n_bar));
...
Rcout<<"p_bar,sampmatA,B,s2,h_bar,ones"<<std::endl;
Rf_PrintValue(wrap(p_bar));
Rf_PrintValue(wrap(sampmatA));
Rf_PrintValue(wrap(sampmatB));
Rf_PrintValue(wrap(s2));
Rf_PrintValue(wrap(h_bar));
Rf_PrintValue(wrap(ones));

double nc=((r*n_bar)-((pow(n_sampA,2)/(r*n_bar))+(pow(n_sampB,2)/(r*n_bar))))/(r-1);  //same as n_bar

Rcout<<"nc"<<std::endl;
Rf_PrintValue(wrap(nc));
...
if (k==660){arichAmat_sub660=arichAmat_sub;
arichBmat_sub680=arichBmat_sub;}
if (k==680){arichAmat_sub680=arichAmat_sub;
arichBmat_sub680=arichBmat_sub;}
if (k==699){arichAmat_sub699=arichAmat_sub;
arichBmat_sub699=arichBmat_sub;}
...
for (int rich = 0; rich<nSNP;rich++){//testing here
count_sub[rich]=0;}
...
Rcout<<"howmanyA_sub"<<std::endl;
Rf_PrintValue(wrap(howmanyA_sub));

Rcout<<"howmanyB_sub"<<std::endl;
Rf_PrintValue(wrap(howmanyB_sub));
...
Rcout<<"tallyAB_sub"<<std::endl;
Rf_PrintValue(wrap(tallyAB_sub));
...
if ((arichAmat_sub((2*kk),i)!=arichAmat_sub(((2*kk)+1),i))&((arichAmat_sub((2*kk),i)==talmat_sub2(j,0))|(arichAmat_sub(((2*kk)+1),i)==talmat_sub2(j,0)))){hetsA_sub=hetsA_sub+1;}
if ((arichBmat_sub((2*kk),i)!=arichBmat_sub(((2*kk)+1),i))&((arichBmat_sub((2*kk),i)==talmat_sub2(j,0))|(arichBmat_sub(((2*kk)+1),i)==talmat_sub2(j,0)))){hetsB_sub=hetsB_sub+1;}
}
...
Rcout<<"talmat_sub2"<<std::endl;
Rf_PrintValue(wrap(talmat_sub2));
...
if (k==660&mgmt_counter==0) {sig_submat660(zcounter,0)=0.01*outfstsig[0];
fst_submat660(zcounter,0)=fst_subvec[0];
}

if (k==699&mgmt_counter==0) {sig_submat699(zcounter,0)=0.01*outfstsig[0];
fst_submat699(zcounter,0)=fst_subvec[0];
}
...
outmat[216]=fst_submat660; //this is the subsampled Fst, 1st column is true
outmat[217]=sig_submat660; //holds significances
outmat[218]=fst_submat699; //this is the subsampled Fst, 1st column is true
outmat[219]=sig_submat699; //holds significances

outmat[220]=arichAmat_sub620;
outmat[221]=arichBmat_sub620;
outmat[222]=arichAmat_sub640;
outmat[223]=arichBmat_sub640;
outmat[224]=arichAmat_sub680;
outmat[225]=arichBmat_sub680;
outmat[226]=tallyhowmanyA;//this is the number of alleles per locus (for whole imulation)
outmat[230]=inputs;
...
#SSBinitA,pop1_prop,NYrs,nsamp,ngen,fishmort,samptype,nsims,N,yrs,mmt,dpow,rmate,ages,popdyn,nSNP,nmig,nmig_prop,SSBinitB,nmig1,nms
#out=run2pops(c(ssba,0.5,700,100,1,.34,0.12,1,1,3000,1000,1,1.15,1,3,1,13,mig,331000,0))#was331000
out=run2pops(c(ssba,0.5,700,100,1,.34,0.12,1,1,3000,1000,1,1.15,1,3,1,13,mig,5457,0))#was331000
save(out,file=paste("silverFF_nomut_20alleles_mig2ways_eqpops_",ssba,"_",mig,"_",seed,".RData",sep="")) 
#save(out,file="silverFF_bugfixd_nomut_spr_2alleles_mig2ways.RData")
